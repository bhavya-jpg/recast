<script lang="ts">
	...
	async function loadThumbnailStrip(path: string) {
		...
		try {
			const count = store.metadata?.duration && store.metadata.duration > 60 ? 12 : 8;
				...
				{#if isLoading}
					<div class="animate-in fade-in flex w-full max-w-lg flex-col items-center gap-5 duration-500">
						<div class="relative flex h-20 w-20 items-center justify-center rounded-[24px] border border-border/70 bg-card shadow-lg">
							<div class="absolute inset-2 rounded-[18px] bg-linear-to-br from-primary/15 via-primary/5 to-transparent"></div>
							<div class="h-9 w-9 animate-spin rounded-full border-2 border-primary border-t-transparent"></div>
						</div>
						<div class="space-y-2 text-center">
							<p class="text-base font-semibold text-foreground">Preparing editor</p>
							<p class="text-sm text-muted-foreground">{loadStage}...</p>
						</div>
						<div class="w-full rounded-full border border-border/60 bg-card/80 p-1">
							<div class="h-2 w-full overflow-hidden rounded-full bg-muted">
								<div class="h-full w-2/3 animate-pulse rounded-full bg-linear-to-r from-primary via-sky-400 to-primary"></div>
							</div>
						</div>
					</div>
				{:else if error}
					<div class="animate-in fade-in flex flex-col items-center gap-4 text-center duration-500">
