...
<div class="flex h-screen w-full flex-col bg-neutral-950 text-white font-sans overflow-hidden">
  <!-- Header -->
  <div class="flex items-center justify-between px-4 pt-3 pb-2 shrink-0">
    <span class="text-[13px] font-semibold tracking-tight text-white/90">
      Select {title}
    </span>
    <button
      onclick={closeWindow}
  ...
  <div class="flex-1 overflow-y-auto px-3 pb-3">
    {#if isLoading}
      <div class="flex items-center justify-center h-20">
        <RefreshCw size={16} class="animate-spin text-white/30" />
      </div>
    {:else if devices.length === 0}
      <div class="flex flex-col items-center justify-center h-20 gap-2">
        {#if isMic}
          <MicOff size={20} class="text-white/15" />
        {:else}
          <CameraOff size={20} class="text-white/15" />
        {/if}
        <p class="text-[11px] text-white/30">No {title.toLowerCase()} devices found</p>
      </div>
    {:else}
      <div class="space-y-1">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              {#if isMic && "isDefault" in device && device.isDefault}
                <span class="text-[9px] text-white/30">Default device</span>
              {/if}
            </div>
  ...
  <div class="flex items-center justify-between px-4 py-3 border-t border-white/6 shrink-0">
    ...
    <button
      onclick={turnOff}
      onmousedown={(e) => e.stopPropagation()}
      class="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-[11px] font-medium text-white/40 hover:text-white/70 hover:bg-white/5 transition-colors"
    >
      {#if isMic}
        <MicOff size={12} />
      {:else}
